...
后续功能进展：0.1.11 已实现依赖 Mimo 的 [网页预览转发](MIMO_WEB_PREVIEW.md)，0.1.12 修复了分片头高位标志被误拒导致的直播约 1 秒中止并完成手机侧实机验收；下文保留 0.1.10 阶段的协议及共存证据。独立订阅仍需完成 [生命周期调查](NATIVE_PREVIEW_LIFECYCLE.md)。
...
| 视频分片 | 数据从 UDP 负载偏移 20 开始；偏移 16 的小端 32 位字段含 8 位帧号（bit0–7）、6 位分片数（bit8–13）、FEC 标志（bit14）及 6 位分片索引（bit15–20）。bit21 以上为实况观测到的标志位（byte2 bit5–6、byte3，如 `0x60`/`0x13`），随帧成组出现、不含分片信息，按此重组的帧经原生媒体头与 NAL 校验后均为合法 I/P 帧，故予接受；仅拒绝 FEC(bit14)、`分片数=0`、`索引≥分片数`，内容合法性由原生媒体头/NAL 校验兜底 |
...
| 网页实时预览 | 后续 0.1.11 已实现 Mimo 视频副本转发、时间轴接续及浏览器封装；脱离 Mimo 的独立预览仍待核实 |
