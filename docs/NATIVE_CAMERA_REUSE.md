...
**已复用相机本地 Binder 服务实现原生录像状态读取和开始/停止录像，不需要 Mimo。设备实测完成 `3 → 1 → 3` 的状态变化，并由相机保存了一段短片。** 相机自身继续管理采集、编码、拍摄设置、文件保存和网络；没有另建录像或无线状态机。原生拍照和网页原生预览仍未接入。用户已允许断开手机后继续开发，并将成果独立记录；需要 Mimo 会话的项目保留 pending。
...
| Mimo 与本地状态读取/录像控制共存 | 本地读取和录像启停均已单独成功；保留原生服务 | 在 Mimo 正常预览和拍摄期间验证并发操作和客户端释放，不把本地结果当作双客户端实测 |
