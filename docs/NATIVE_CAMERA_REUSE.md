Action Control 原生相机复用进度，版本 0.1.10，已部署；更新于 2026-09-19。部署及短片验收完成于 2026-09-18。

2026-09-19 补充：用户重新连接 Mimo 后，已完成本地原生读取、录像启停与该会话共存的服务/传输层验证，并完成 Mimo 预览离线重组和解码。新增工具、真实预览规格及录像切换的时钟问题见 [Mimo 预览协议与验收](MIMO_PREVIEW_PROTOCOL.md)。本轮未更新设备程序，网页原生预览仍未接入。

**已复用相机本地 Binder 服务实现原生录像状态读取和开始/停止录像，不需要 Mimo。设备实测完成 `3 → 1 → 3` 的状态变化，并由相机保存了一段短片。** 相机自身继续管理采集、编码、拍摄设置、文件保存和网络；没有另建录像或无线状态机。原生拍照和网页原生预览仍未接入。用户已允许断开手机后继续开发，并将成果独立记录；尚未完成的 Mimo 会话项目保留 pending。
...
| 默认页面 | 显示录像状态、读取时间、开始/停止按钮和上次操作结果，每 5 秒刷新；原生预览仍标“未接入”，独立采集在默认折叠的高级入口内 |
...
网页原生预览已有本次 Mimo 会话的离线重组证据，仍需核实持续获取数据的生命周期、处理录像切换时钟并接入浏览器封装。当前没有已验证的 RTSP 地址，也不能承诺多客户端订阅或任意格式免转码。
