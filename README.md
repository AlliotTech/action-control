...
- 查看设备、存储、网络、进程和服务状态
- 只读查看原生相机服务状态；高级独立采集可输出 MPEG-TS / H.264 实时画面并保存快照
...
相机页默认展示原生服务状态，独立采集入口位于“高级：独立采集”。原生录像状态、网页原生预览和拍摄控制尚未接入；服务运行不代表正在录像或预览。全局的“独立采集未启动”仅描述 Action Control 自己的管线。
