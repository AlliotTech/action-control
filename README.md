...
- 查看设备、存储、网络、进程和服务状态
- 复用相机本地服务，只读查看原生录像状态及服务状态；高级独立采集可输出 MPEG-TS / H.264 实时画面并保存快照
...
相机页默认展示原生录像和服务状态，独立采集入口位于“高级：独立采集”。从 0.1.9 起，录像状态通过相机原生客户端库和本地 Binder 服务读取，不需要 Mimo。只有已验证的录像中、空闲代码会显示明确结果；未知状态、读取失败和固件不匹配均不推测。网页原生预览和拍摄控制仍未接入。全局的“独立采集未启动”仅描述 Action Control 自己的管线。
