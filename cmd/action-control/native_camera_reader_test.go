...
func TestNativeRecordingUsesOnlyVerifiedCodes(t *testing.T) {
	for _, code := range []int{-1, 0, 1, 2, 3, 4, 5, 100} {
		t.Run(fmt.Sprint(code), func(t *testing.T) {
			observation, err := decodeNativeObservation([]byte(fmt.Sprintf(`{"schema":1,"status":"ok","camera_id":0,"camera_amount":1,"record_state":%d,"capture_state":0}`, code)))
			if err != nil {
				t.Fatal(err)
			}
...
func TestNativeReaderRejectsIncompleteOrContaminatedOutput(t *testing.T) {
	good := `{"schema":1,"status":"ok","camera_id":0,"camera_amount":1,"record_state":1,"capture_state":0}`
...
func TestNativeReaderKillsHungChildAndDiscardsPartialOutput(t *testing.T) {
	...
	script := "#!/bin/sh\nprintf '%s\\n' '{\"schema\":1,\"status\":\"ok\",\"camera_id\":0,\"camera_amount\":1,\"record_state\":1,\"capture_state\":0}'\nexec /bin/sleep 30\n"
